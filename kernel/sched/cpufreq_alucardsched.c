...
static void acgov_get_util(unsigned long *util, unsigned long *max, u64 time)
{
	...
	*util = min(rq->cfs.avg.util_avg + rt, max_cap);
	...
	if (!walt_disabled && sysctl_sched_use_walt_cpu_util)
		*util = boosted_cpu_util(cpu, NULL);
#endif
	*max = max_cap;
}
...
static int alucardsched_init(struct cpufreq_policy *policy) {
    return acgov_init(policy);
}

static void alucardsched_exit(struct cpufreq_policy *policy) {
    acgov_exit(policy);
}

static int alucardsched_start(struct cpufreq_policy *policy) {
    return acgov_start(policy);
}

static void alucardsched_stop(struct cpufreq_policy *policy) {
    acgov_stop(policy);
}

static void alucardsched_limits(struct cpufreq_policy *policy) {
    acgov_limits(policy);
}
...
struct cpufreq_governor alucardsched_gov = {
	.name = "alucardsched",
	.init = alucardsched_init,
	.exit = alucardsched_exit,
	.start = alucardsched_start,
	.stop = alucardsched_stop,
	.limits = alucardsched_limits,
	.owner = THIS_MODULE,
};
